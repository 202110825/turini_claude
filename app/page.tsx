        ...
        <section className="app-main">
          ...
          {view === "home" && (
            <div className="screen home-screen">
              ...
              <section className="hero-card">
                ...
                <div className="hero-mascot"><Turini state="idle" className="turini-hero" /><span className="spark spark-one">✦</span><span className="spark spark-two">◆</span></div>
              </section>
          ...
          {view === "category" && (
            <div className="screen category-screen">
              ...
              <div className="category-list">{CATEGORIES.map((category) => {
                ...
                return <article className={`category-card ${category.color}`} key={category.name}><button className="category-main" onClick={() => { setActiveCategoryName(category.name); navigate("learn"); }}><span className="category-icon">{category.icon}</span><div><div className="category-title-row"><h2>{category.name}</h2><span>Lv. {categoryLevel}</span></div><p>{category.copy}</p><div className="progress-track"><span style={{ width: `${solved / 120 * 100}%` }} /></div><small>{solved} / 120문항 완료 · 레벨 학습 보기</small></div><b>›</b></button><div className="difficulty-row">{(["초급", "중급", "고급"] as Difficulty[]).map((difficulty) => <button key={difficulty} onClick={() => startCategory(category.name, difficulty)}>{difficulty}</button>)}</div></article>;
              })}</div>
            </div>
          )}
          ...
          {view === "portfolio" && (
            <div className="screen portfolio-screen">
              ...
              <section className="portfolio-intro"><div><span className="pill">핵심 기능</span><h2>비중을 입력하고<br />투리니의 코칭 받기</h2><p>개별 종목 추천이 아닌 자산배분 학습용 분석이에요.</p></div><div className="portfolio-mascot-frame"><Turini state="idle" className="turini-portfolio" /></div></section>
