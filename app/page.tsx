...
import { updateStudyStreak } from "./streak";
import { DEFAULT_TURINI_STYLE, normalizeTuriniStyle, StyledTurini, TuriniStyleCustomizer, type TuriniStyle } from "./turini-style";

type View = "home" | "learn" | "category" | "portfolio" | "assets" | "profile";
...
export default function Home() {
  ...
  const [selectedDifficulty, setSelectedDifficulty] = useState<Difficulty>("초급");
  ...
  const [avatar, setAvatar] = useState<TuriniStyle>(DEFAULT_TURINI_STYLE);
  ...
  const hydrateAccount = (payload: AccountPayload) => {
    ...
    setAvatar(normalizeTuriniStyle(savedPortfolio.avatar));
  ...
  const resetClientState = () => {
    ...
    setAvatar({ ...DEFAULT_TURINI_STYLE });
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const response = await fetch("/api/account", {
          ...
          body: JSON.stringify({
            progress,
            portfolio: { allocation, amount, goal, horizon, result: portfolioResult, ruleVersion: PORTFOLIO_RULE_VERSION, planner, avatar },
          }),
        });
  ...
  }, [account, accountStateReady, allocation, amount, avatar, goal, horizon, loading, planner, portfolioResult, progress]);
  ...
  const finishSession = (finished: QuizSession) => {
    ...
    setProgress((current) => {
      const streak = finished.mode === "diagnosis" ? { streak: current.streak, lastStudyDate: current.lastStudyDate } : updateStudyStreak(current.streak, current.lastStudyDate);
      return {
      ...current, ...streak,
  ...
  if (loading) {
    return <main className="loading-screen"><Turini state="reading" className="turini-loading" /><h1>투리니가 문제를 준비하고 있어요!</h1><div className="loading-track"><span /></div></main>;
  }
    ...
    return (
      <main className="onboarding-stage">
        <section className="onboarding-card">
          ...
          <Turini state="idle" className="turini-onboarding" />
      ...
      <main className="quiz-stage">
        <section className="quiz-phone" data-question-id={question.id} data-concept-id={conceptKey(question)} data-review-kind={question.reviewKind || "new"}>
          ...
          <div className={`quiz-mascot ${answered ? "answered" : "waiting"}`}>
            {answered
              ? <Turini state={answerCorrect ? "correct" : "wrong"} className="turini-quiz" reactKey={`${question.id}-${session.index}`} />
              : <Turini state="thinking" className="turini-quiz" reactKey={question.id} />}
    ...
    return (
      <main className="result-stage">
        <section className="result-card-page">
          ...
          <Turini state="celebrate" className="turini-lesson-result" reactKey={result.correct} />
  ...
  return (
    <main className="app-bg">
      <div className="app-shell">
        <section className="app-main">
          ...
          {view === "home" && (
            <div className="screen home-screen">
              ...
              <section className="hero-card">
                ...
                <div className="hero-mascot"><StyledTurini style={avatar} className="styled-hero" /><span className="spark spark-one">✦</span><span className="spark spark-two">◆</span></div>
              </section>
              <button className="future-banner" onClick={() => navigate("assets")}>
                ...
                <Turini state="thinking" className="turini-future" decorative />
              </button>
          ...
          {view === "learn" && (
            <div className="screen learn-screen">
              <PageTitle eyebrow="LEARNING ROADMAP" title={`${activeCategory.name} 성장 로드맵`} copy="난이도를 고른 뒤 원하는 구간부터 학습해요. 전체 성장 흐름도 함께 확인할 수 있어요." />
              <section className="learning-banner"><div><span>{activeCategory.name} 레벨</span><h2>Lv. {activeCategoryLevel}</h2><div className="progress-track"><span style={{ width: `${Math.min(100, activeCategorySolved / QUESTIONS_PER_CATEGORY * 100)}%` }} /></div><small>{activeCategoryCompletedLessons} / {MAX_CATEGORY_LEVEL} 레슨 완료</small></div><Turini state="reading" className="turini-learning" /></section>
              <section className="difficulty-picker"><div><p className="eyebrow">난이도 선택</p><h2>어디서부터 시작할까요?</h2></div><div>{(["초급", "중급", "고급"] as Difficulty[]).map(difficulty => <button key={difficulty} className={selectedDifficulty === difficulty ? "active" : ""} onClick={() => setSelectedDifficulty(difficulty)}><b>{difficulty}</b><span>{difficulty === "초급" ? "기초 개념" : difficulty === "중급" ? "원리 이해" : "실전 판단"}</span></button>)}</div></section>
              <div className="learning-roadmap">{Array.from({ length: MAX_CATEGORY_LEVEL }, (_, index) => index + 1).map((level) => {
                ...
                const difficulty: Difficulty = level <= 4 ? "초급" : level <= 8 ? "중급" : "고급";
                return <article className={`roadmap-step ${completed ? "completed" : ""} ${current ? "current" : ""} ${locked ? "locked" : ""} ${difficulty === selectedDifficulty ? "focused" : "dimmed"}`} key={level}><span className="roadmap-marker">{completed ? "✓" : locked ? "🔒" : level}</span><div><small>{difficulty} · STEP {level}</small><h3>{activeCategory.name} 핵심 개념 {level}</h3><p>10문제로 개념을 익히고 바로 복습해요.</p></div><button onClick={() => !locked && startLesson(activeCategory.name, level)} disabled={locked}>{completed ? "다시 학습" : current ? "시작" : locked ? "잠김" : "학습"}</button></article>;
              })}</div>
            </div>
          )}
          ...
          {view === "category" && (
            <div className="screen category-screen">
              <PageTitle eyebrow="LEARNING" title="금융 학습" copy="카테고리를 고르고 레벨별 학습이나 난이도별 연습을 시작해요." />
              <div className="category-list">{CATEGORIES.map((category) => {
                ...
                return <article className={`category-card ${category.color}`} key={category.name}><button className="category-main" onClick={() => { setActiveCategoryName(category.name); setSelectedDifficulty(categoryLevel <= 4 ? "초급" : categoryLevel <= 8 ? "중급" : "고급"); navigate("learn"); }}><span className="category-icon">{category.icon}</span><div><div className="category-title-row"><h2>{category.name}</h2><span>Lv. {categoryLevel}</span></div><p>{category.copy}</p><div className="progress-track"><span style={{ width: `${solved / 120 * 100}%` }} /></div><small>{solved} / 120문항 완료 · 난이도 선택하기</small></div><b>›</b></button></article>;
              })}</div>
            </div>
          )}
          ...
          {view === "portfolio" && (
            <div className="screen portfolio-screen">
              <PageTitle eyebrow="MY PORTFOLIO" title="내 포트폴리오 설계" copy="여섯 자산의 현재 비중을 입력하면 성향 적합도와 조정 방향을 바로 확인해요." />
              <section className="portfolio-intro"><div><span className="pill">핵심 기능</span><h2>비중을 입력하고<br />투리니의 코칭 받기</h2><p>개별 종목 추천이 아닌 자산배분 학습용 분석이에요.</p></div><div className="portfolio-mascot-frame"><StyledTurini style={avatar} className="styled-portfolio" /></div></section>
                  ...
                  <div className="asset-inputs">
                    {ASSETS.map((asset) => <div className="asset-item" key={asset.key}>
                      <div className="asset-row">
                        ...
                        <span className="percent-stepper"><button type="button" aria-label={`${asset.label} 1% 줄이기`} onClick={() => { setAllocation({ ...allocation, [asset.key]: clamp(toPercent(allocation[asset.key]) - 1) / 100 }); setPortfolioResult(null); }}>−</button><span className="percent-input"><input aria-label={`${asset.label} 비중`} type="number" min="0" max="100" step="0.1" value={toPercent(allocation[asset.key])} onChange={(event) => { setAllocation({ ...allocation, [asset.key]: clamp(Number(event.target.value)) / 100 }); setPortfolioResult(null); }} />%</span><button type="button" aria-label={`${asset.label} 1% 늘리기`} onClick={() => { setAllocation({ ...allocation, [asset.key]: clamp(toPercent(allocation[asset.key]) + 1) / 100 }); setPortfolioResult(null); }}>＋</button></span>
                      </div>
              ...
              <section className="portfolio-options card-block"><div><label>투자 목적 <small>(기록용)</small><select value={goal} onChange={(event) => setGoal(event.target.value)}><option>장기 자산 증식</option><option>주택·목돈 마련</option><option>은퇴 준비</option><option>단기 여유자금 운용</option></select><em>현재 점수·조정 계산에는 반영되지 않아요.</em></label><label>투자 기간<select value={horizon} onChange={(event) => { setHorizon(event.target.value); setPortfolioResult(null); }}><option>1년 미만</option><option>1~3년</option><option>3~5년</option><option>5년 이상</option></select></label><label>총 투자금액 <small>(기록용)</small><div className="money-input"><input type="number" min="0" step="100000" value={amount} onChange={(event) => setAmount(Math.max(0, Number(event.target.value) || 0))} /><span>원</span></div><em>리밸런싱 결과에는 금액이 표시되지 않아요.</em></label></div><div className={`portfolio-submit-bar ${validateAllocation(allocation) ? "ready" : ""}`}><span><b>{sumAllocation(allocation)}%</b><small>{validateAllocation(allocation) ? "분석할 준비가 됐어요" : sumAllocation(allocation) < 100 ? `${Math.round((100 - sumAllocation(allocation)) * 10) / 10}% 더 입력해 주세요` : `${Math.round((sumAllocation(allocation) - 100) * 10) / 10}% 줄여 주세요`}</small></span><button className="primary-button" disabled={!validateAllocation(allocation)} onClick={runPortfolioAnalysis}>{validateAllocation(allocation) ? "포트폴리오 분석하기" : "합계를 100%로 맞춰주세요"}</button></div></section>
          ...
          {view === "profile" && (
            <div className="screen profile-screen">
              <section className="profile-hero"><div className="profile-mascot-frame"><StyledTurini style={avatar} className="styled-profile" /></div><div><p className="eyebrow">MY PROFILE</p><h1>{account.username}</h1><span>나만의 금융 학습 기록</span></div></section>
              <TuriniStyleCustomizer value={avatar} onChange={setAvatar} />
        ...
        <nav className="mobile-nav" aria-label="주요 메뉴">{(["home", "category", "portfolio", "assets", "profile"] as View[]).map((item) => <NavButton key={item} item={item} active={view === item || (item === "category" && view === "learn")} onClick={() => navigate(item)} />)}</nav>
      </div>
    </main>
  );
}
  ...
  return (
    <main className="auth-stage">
      <section className="auth-card">
        <div className="auth-brand"><Turini state="idle" className="turini-auth" /><div><p className="eyebrow">WELCOME TO TURINI</p><h1>나만의 금융 학습을<br />이어서 시작해요</h1><p>아이디별로 진단 결과와 학습 기록을 안전하게 보관해요.</p></div></div>
...
function WealthPlanner({ state, setState }: { state: WealthPlannerState; setState: Dispatch<SetStateAction<WealthPlannerState>> }) {
  ...
  return <>
    <section className="asset-planner-hero">
      ...
      <div className="asset-planner-mascot"><Turini state="thinking" className="turini-planner" /></div>
    </section>
    ...
    {tab === "future" ? <>
      ...
      <div className="coach-nudge"><Turini state="reading" className="turini-nudge" decorative /><p><b>투리니 팁</b> 적금과 투자는 소비가 아니라 내 자산으로 옮긴 돈이에요. 생활지출과 분리해서 보면 이번 달의 성장이 더 잘 보여요.</p></div>
    </>}
  </>;
}
...
function NavButton({ item, active, onClick }: { item: View; active: boolean; onClick: () => void }) {
  const meta: Record<View, { label: string }> = {
    home: { label: "홈" }, learn: { label: "학습" }, category: { label: "학습" }, portfolio: { label: "포트폴리오" }, assets: { label: "자산" }, profile: { label: "마이" },
  };
...
function PortfolioResults({ result, allocation, tab, setTab, aiFeedback, aiFeedbackLoading, aiFeedbackError, retryAiFeedback }: { result: PortfolioResult; allocation: Allocation; tab: "summary" | "rebalance" | "detail" | "coach"; setTab: (tab: "summary" | "rebalance" | "detail" | "coach") => void; aiFeedback: AIFeedback | null; aiFeedbackLoading: boolean; aiFeedbackError: string; retryAiFeedback: () => void }) {
  ...
  return <section id="portfolio-result" className="portfolio-result card-block">
    <div className="result-hero"><div><p>내부 종합점수 · {result.scoreMax}점 만점</p><strong>{result.score}<small>점</small></strong><span>{result.scoreLabel}</span><div className="stars">{"★".repeat(starCount)}{"☆".repeat(5 - starCount)}</div></div><Turini state="celebrate" className="turini-score" reactKey={result.score} /></div>
    ...
    {tab === "summary" && <div className="tab-panel"><div className="coach-banner"><Turini state="reading" className="turini-coach" decorative /><div><b>투리니 코치의 한마디!</b><p>{result.coach}</p></div></div><div className="analysis-columns"><article className="good"><h3>강점</h3>{result.strengths.length ? <ul>{result.strengths.map((item) => <li key={item}>{item}</li>)}</ul> : <p>현재 강점 기준을 충족한 항목이 없어요.</p>}</article><article className="care"><h3>개선하면 좋은 점</h3>{result.cautions.length ? <ul>{result.cautions.map((item) => <li key={item}>{item}</li>)}</ul> : <p>현재 규칙에서 별도로 주의할 점이 없어요.</p>}</article></div></div>}
    {tab === "rebalance" && <div className="tab-panel"><div className="target-chart"><div className="allocation-donut small" style={targetChartStyle}><span>예시</span></div><div><h3>학습용 조정 예시</h3><p>현재 비중과 성향별 목표의 중간 지점을 비율로만 보여드려요.</p></div></div>{result.rebalancingActions.length ? <div className="rebalance-table"><div className="table-head"><span>자산</span><span>현재</span><span>예시</span><span>방향</span></div>{result.rebalancingActions.map((item) => { const asset = ASSETS.find((candidate) => candidate.key === item.asset)!; return <div key={asset.key}><strong><i style={{ background: asset.color }} />{asset.label}</strong><span>{toPercent(allocation[asset.key])}%</span><span>{toPercent(result.target[asset.key])}%</span><b className={item.delta > 0 ? "buy" : "sell"}>{item.delta > 0 ? `늘리기 +${item.delta}%p` : item.delta < 0 ? `줄이기 ${item.delta}%p` : "유지"}</b></div>; })}</div> : <p className="fine-print">5%p 이상 차이가 나는 자산군이 없어요.</p>}{result.residualItems.length ? <div className="residual-list"><b>표에 표시되지 않은 작은 차이</b>{result.residualItems.map((item) => { const asset = ASSETS.find((candidate) => candidate.key === item.asset)!; return <span key={item.asset}>{asset.label} {item.delta > 0 ? "+" : ""}{item.delta}%p</span>; })}</div> : null}<p className="fine-print">수수료·세금·개별 상품 특성은 반영하지 않은 자산배분 학습용 계산이에요.</p></div>}
